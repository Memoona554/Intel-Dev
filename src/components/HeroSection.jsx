import React, { useState, useEffect } from "react";
import 'swiper/swiper-bundle.css';

const slides = [
    {
        heading: "Build Decentralized Solutions with",
        highlight: "Blockchain Development Services",
        description:
            "We offer a wide range of reliable and tailored blockchain development services for your business. Build a decentralized blockchain network with Vaival for more transparency, efficiency, and automation.",
        buttonText: "Let’s Connect",
        buttonLink: "#calendly",
    },
    {
        heading: "Build Software Solutions with",
        highlight: "Web Development Services",
        description:
            "We offer a wide range of reliable and tailored blockchain development services for your business. Build a decentralized blockchain network with Vaival for more transparency, efficiency, and automation.",
        buttonText: "Let’s Connect",
        buttonLink: "#calendly",
    },
    // Add more slides if needed
];

const HeroSection = () => {
    const [currentSlide, setCurrentSlide] = useState(0);
    const [fadeIn, setFadeIn] = useState(true); // Control fade-in/out effect

    // Cycle through slides
    useEffect(() => {
        const interval = setInterval(() => {
            setFadeIn(false);  // Start fading out
            setTimeout(() => {
                setCurrentSlide((prev) => (prev + 1) % slides.length); // Change slide
                setFadeIn(true);  // Start fading in
            }, 500);  // Wait until fade-out animation is done
        }, 5000);  // Change slide every 5 seconds

        return () => clearInterval(interval);
    }, []);

    return (
        <div
        className="w-full h-[98vh] relative  lg:bg-cover bg-no-repeat lg:bg-center"
        style={{
          backgroundImage: "url('/bannerimage.webp')",
          
        }}
      >

        <div className="w-full h-full bg-cover bg-center lg:pl-[10rem] flex items-center lg:justify-start justify-center text-white px-4">
          <div
            className={`absolute text-center lg:text-left max-w-3xl transition-opacity duration-500 ease-in-out ${
              fadeIn ? 'opacity-100' : 'opacity-0'
            }`}
          >
            <h2 className="text-3xl lg:p-0 p-2 md:text-5xl font-bold mb-4">
              {slides[currentSlide].heading}{" "}
              <strong className="text-blue-400">{slides[currentSlide].highlight}</strong>
            </h2>
            <p className="text-lg lg:p-0 p-2 mb-6">{slides[currentSlide].description}</p>
            <a
              href={slides[currentSlide].buttonLink}
              className="inline-block bg-blue-400 text-white font-semibold px-6 py-3 rounded-full shadow hover:bg-blue-300 transition"
            >
              {slides[currentSlide].buttonText}
            </a>
          </div>
        </div>
      </div>
      
    );
};

export default HeroSection;
